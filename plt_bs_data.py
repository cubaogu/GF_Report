...
stock_list = ['000968.SZ','601918.SH','600408.SH','600397.SH','601011.SH','601001.SH','601101.SH',
 ...
 '600188.SH',	 '600348.SH',	 '900948.SH',	 '600157.SH',	 '600121.SH',	 '601088.SH',	 '601898.SH']#这是删掉2007-2015年有缺失数据的公司之后的股票，共31支 再删掉缺失固定资产和在建工程的三只，共28只,再删了一只重组的？27只


percent_indicator_list = ['销售毛利率(%)','销售费用率(%)','管理费用率(%)','财务费用率(%)','销售净利率(%)','资产负债率(%)']
rate_indicator_list = ['现金比率','流动比率','速动比率','应收账款周转率','固定资产周转率','存货周转率','总资产周转率','Z值','有息债务/EBITDA']
...
'归属母公司股东的净利润(亿)','购建固定无形长期资产支付现金(亿)','销售商品劳务收到现金(亿)','固定资产(亿)','在建工程(亿)','EBITDA（亿）']
...
for stock in stock_list:
	for i in range(2007,2016):
		td = w.wss(stock, "ebitda","rptDate=%s1231;rptType=1"%i).Data #fix_assets
		all_origin_data.append(td[0][0])
stockDF['EBITDA'] = all_origin_data #固定资产
w.stop()

stockDF['EBITDA（亿）'] = stockDF['EBITDA']/10**8
del stockDF['EBITDA']

stockDF['有息债务/EBITDA'] = stockDF['带息债务(亿)']/stockDF['EBITDA（亿）'] 
...
t_indicator_list = ['有息债务/EBITDA','经营现金流净额(亿)']
for i,indicator in enumerate(t_indicator_list):
	...
	tmp_stockDF = stockDF[stockDF['股票代码'] != '601666.SH'] #把极值给剔除了(针对有息债务/ebitda)
